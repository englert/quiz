...
def oct_to_bin(oct_num):
    return bin(int(oct_num, 8))[2:]  # Bináris szám, a '0b' prefix nélkül
...
def generate_quiz(num_questions=30):
    ...
    for i in range(num_questions):
        length = random.choice([2, 3, 4])  # Véletlenszerűen 2-4 karakter hosszú oktális szám
        oct_num = generate_oct_number(length)
        questions.append((i + 1, oct_num, oct_to_bin(oct_num)))  # (sorszám, oktális szám, bináris válasz)
    return questions
...
def load_quiz(filename='quiz.txt'):
    ...
    for line in lines:
        ...
        # Feladat sor feldolgozása (pl. "1. 0o12 = 1010")
        if line.startswith("Checksum:") or line.startswith("Full path:"):
            continue
        
        parts = line.split("=")
        question_part = parts[0].strip()  # "1. 0o12"
...
def test_question_1():
    ...
    correct_answer = oct_to_bin(oct_num)
